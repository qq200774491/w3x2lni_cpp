...
std::filesystem::path DefaultOutputPath(
  ...
  if (core::FilesystemUtils::IsDirectory(input_path)) {
    return input_path.parent_path() /
           (input_path.filename().string() + "_slk.w3x");
  }
  return input_path.parent_path() / (input_path.stem().string() + "_slk.w3x");
}
...
std::string SlkCommand::Name() const { return "slk"; }

std::string SlkCommand::Description() const {
  return "Convert a map to a first-stage Slk-style packed map";
}

std::string SlkCommand::Usage() const {
  return "slk <input_map_dir|input_map.w3x|input_map.w3m> "
         "[output_map.w3x|output_map.w3m]";
}

core::Result<void> SlkCommand::Execute(const std::vector<std::string>& args) {
  if (args.empty()) {
  ...
  const std::filesystem::path output_path = args.size() >= 2
                                                ? std::filesystem::path(args[1])
                                                : DefaultOutputPath(input_path);

  auto& logger = core::Logger::Instance();
  logger.Info("Converting '{}' -> '{}' as Slk packed map",
  ...
  if (core::FilesystemUtils::IsDirectory(input_path)) {
    ...
    W3X_ASSIGN_OR_RETURN(
        ...
        parser::w3x::PackMapDirectory(input_path, output_path,
                                      BuildSlkPackOptions(config)));
    std::cout << "Slk map written to: " << output_path.string() << std::endl;
    std::cout << "Packed " << packed_files << " file(s)." << std::endl;
    return {};
  }

  TemporaryDirectory unpacked_dir("w3x_toolkit_slk");
  W3X_RETURN_IF_ERROR(
  ...
  W3X_ASSIGN_OR_RETURN(auto config, LoadRuntimeConfig(unpacked_dir.path()));
  W3X_ASSIGN_OR_RETURN(
      auto packed_files,
