...
const std::unordered_map<std::string, std::string> JassToLuaConverter::kTypeMap{
    ...
    {"string", "string"},   {"handle", "userdata"},{"code", "function"},
    {"nothing", "nil"},
};
...
std::string JassToLuaConverter::Indent(int level) const {
  std::string r;
  for (int i = 0; i < level; ++i) r += options_.indent;
  return r;
}
...
std::string JassToLuaConverter::TranslateExpression(
  ...
  while (pos < tokens.size()) {
    ...
    if (tok.type == TT::kOperator) {
      out << " " << TranslateOperator(tok.text) << " "; ++pos; continue;
    }
    if (tok.type == TT::kRawCodeLiteral) {
      ...
      if (raw.size() >= 2 && raw.front() == '\'' && raw.back() == '\'') {
        ...
        uint32_t val = 0;
        for (char c : chars) val = (val << 8) | static_cast<uint8_t>(c);
        out << val;
      } else {
        out << raw;
      }
...
std::string JassToLuaConverter::TranslateCondition(
  ...
  while (pos < tokens.size() && !IsKw(tokens, pos, "then")) {
    if (tokens[pos].type == TT::kNewline) { ++pos; continue; }
...
std::string JassToLuaConverter::TranslateGlobalsBlock(
  ...
  while (pos < tokens.size() && !IsKw(tokens, pos, "endglobals")) {
    SkipWs(tokens, pos);
    if (pos >= tokens.size() || IsKw(tokens, pos, "endglobals")) break;
    if (tokens[pos].type == TT::kComment) {
      out << tokens[pos].text << "\n"; ++pos; continue;
    }

    if (IsKw(tokens, pos, "constant")) ++pos;
    if (pos < tokens.size() &&
  ...
  if (IsKw(tokens, pos, "endglobals")) ++pos;
  out << "-- end globals\n";
  return out.str();
}
...
std::string JassToLuaConverter::TranslateFunctionBody(
  ...
  while (pos < tokens.size()) {
    SkipWs(tokens, pos);
    if (pos >= tokens.size() || tokens[pos].type == TT::kEndOfFile) break;
    ...
    if (IsKw(tokens, pos, "local")) {
      ++pos;
      if (pos < tokens.size()) ++pos;  // skip type
      bool arr = IsKw(tokens, pos, "array");
      if (arr) ++pos;
      if (pos >= tokens.size()) break;
      std::string vn = tokens[pos].text; ++pos;
      if (pos < tokens.size() && tokens[pos].type == TT::kOperator &&
          tokens[pos].text == "=") {
        ++pos;
        std::string expr = TranslateExpression(tokens, pos);
    ...
    if (IsKw(tokens, pos, "loop")) {
      ...
      out << TranslateFunctionBody(tokens, pos, indent_level + 1);
      if (IsKw(tokens, pos, "endloop")) ++pos;
      out << ind << "end\n";
      continue;
    }
...
core::Result<std::string> JassToLuaConverter::TranslateTokens(
  ...
  while (pos < tokens.size()) {
    ...
    if (IsKw(tokens, pos, "function")) {
      ...
      if (IsKw(tokens, pos, "returns")) { ++pos; if (pos < tokens.size()) ++pos; }
      ...
      if (IsKw(tokens, pos, "endfunction")) ++pos;
      out << "end\n\n";
      continue;
    }
